...
public class XACMLPolicyImporter extends XACMLPolicyAggregator {
	...
	@Override
	public CallbackResult onAttribute(Object parent, Object container, com.att.research.xacml.api.Attribute attribute) {
		if (importAttributes) {
	...
	protected int doImportAttributes() {
		...
		//
		JPAContainer<com.att.research.xacml.admin.jpa.Attribute> attributes = ((XacmlAdminUI)UI.getCurrent()).getAttributes();
		//
		// Get mutable entity providers for these.
		//
		JPAContainer<com.att.research.xacml.admin.jpa.Category> categories = new JPAContainer<Category>(Category.class);
		...
		JPAContainer<com.att.research.xacml.admin.jpa.Datatype> datatypes = new JPAContainer<Datatype>(Datatype.class);
						...
						if ((values.size() > 1) && (dt.equals(XACML3.ID_DATATYPE_BOOLEAN) == false)) {
							...
							for (AttributeValue<?> value : values) {
								...
								if (currentAttribute != null && this.attributeOption == OPTION.UPDATE_EXISTING) {
									//
									// We are updating an existing. We can really only do this for enumerations,
									// its impossible to resolve a regular expression or range.
									//
									if (newAttribute.getConstraintType().getConstraintType().equals(ConstraintType.ENUMERATION_TYPE)) {
										//
