...
import com.att.research.xacml.admin.util.OnDemandFileDownloader;
import com.att.research.xacml.admin.util.OnDemandFileDownloader.OnDemandStreamResource;
import com.att.research.xacml.admin.util.XACMLPolicyImporter;
import com.att.research.xacml.admin.view.windows.GitPushWindow;
...
public class PolicyWorkspace extends CustomComponent implements DropHandler, OnDemandStreamResource {
			...
			public void windowClose(CloseEvent event) {
				...
				try {
					if (self.isOverwritingAPolicy(newPolicyPath, oldPolicyFile) == false) {
						//
						// We are not replacing an old file, check if we are overwriting
						// an existing file.
						//
	...
	protected boolean isOverwritingAPolicy(Path newPolicyPath, Path oldPolicyPath) throws Exception {
		//
		// Check to see if we were editing an existing file
		//
		if (oldPolicyPath != null) {
			//
			// First we must check if the new file actually exists. Then check if
			// we are overwriting the original old file
			//
